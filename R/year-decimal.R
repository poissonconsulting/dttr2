...
#' @describeIn dtt_year_decimal Get numeric vector of decimal year values for a Date vector
#' @export
dtt_year_decimal.Date <- function(x, ...) {
  dtt_year(x) + (dtt_doy(x) - 1) / dtt_days_in_year(x)
}

#' @describeIn dtt_year_decimal Get numeric vector of decimal year values for a POSIXct vector
#' @export
