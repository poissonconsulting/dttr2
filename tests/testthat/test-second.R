...
test_that("dtt_second.POSIXct", {
  ...
  expect_identical(dtt_second(as.POSIXct(c("2001-01-01 00:00:31", "2001-01-02 00:01:33"), tz = "UTC")), 
                   c(31L, 33L))
  expect_identical(dtt_second(c(as.POSIXct("2001-01-01 00:00:31", tz = "UTC"), NA_POSIXct_)),
                   c(31L, NA_integer_))
  expect_identical(dtt_second(as.POSIXct("2001-01-01 00:00:31", tz = "Etc/GMT+8")), 31L)
})

test_that("dtt_second.hms", {
  expect_identical(dtt_second(hms::as.hms(1)[-1]), integer(0))
  expect_identical(dtt_second(NA_hms_), NA_integer_)
  expect_identical(dtt_second(hms::as.hms(1)), 1L)
  expect_identical(dtt_second(hms::as.hms(1.999)), 1L)
  expect_identical(dtt_second(hms::as.hms(-0.001)), 59L)
  expect_identical(dtt_second(hms::as.hms(-1)), 59L)
  expect_identical(dtt_second(hms::as.hms("20:30:32")), 32L)
  expect_identical(dtt_second(hms::as.hms(-63)), 57L)
  expect_identical(dtt_second(c(hms::as.hms(1), NA_hms_)), c(1L, NA_integer_))
  expect_identical(dtt_second(hms::as.hms("24:00:00")), 0L)
})
...
test_that("dtt_second<-.hms", {
  ...
  dtt_second(x) <- 30L
  expect_identical(x, hms::as.hms("12:13:30"))
